...
TokenArr* lex_code(char* file_name) {
    ...
    for (int i = 0; i < result_tok_arr->str_bucket.bucket_capacity; i++) {
        ...
        // skip comment
        else if (code_string[i] == '-' && code_string[i+1] == '-')                       { skip_line_comment(code_string, &i, result_tok_arr->str_bucket.bucket_capacity);                    continue; }
        if (code_string[i] == '|' && code_string[i+1] == '"' && code_string[i+2] == '"') { skip_block_comment(code_string, &i, result_tok_arr->str_bucket.bucket_capacity);                   continue; }
...
void skip_line_comment(char* code_str, int* cur_index, int max_index) {
    (*cur_index)++;
...
void skip_block_comment(char* code_str, int* cur_index, int max_index) {
    int open_comments = 1;
    (*cur_index) += 2;
    
    while (open_comments) {
        if ((*cur_index)+2 >= max_index) return;
